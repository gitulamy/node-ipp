...
const serializer = (msg) => {
  ...
  const attr = (group, name, obj) => {
    ...
    values.forEach((value, i) => {
      ...
      try {
        ...
        throw `(serialize:writeValue) tag:${tag}, name:${name}, value:${value}, syntax:${syntax}, syntax2:${syntax2}, ${e}`;
      }
    });
  };
  ...
  const writeUnsupportedAttrTag = (tag) => {
    // support writing multiple sets of the same group with an Array
    const groupsOrAttrs = msg[tag];
  ...
  writeUnsupportedAttrTag('unsupported-attributes-tag');
